...
The primary users are football fans who want to understand player performance beyond just goals and assists, data-curious enthusiasts interested in exploring advanced metrics like expected goals and progressive passes, and amateur analysts looking to compare players across different leagues.

Secondary users include football bloggers and content creators, and students studying sports analytics.


TASKS USERS WILL PERFORM

Users will be able to compare players by selecting two players and viewing side-by-side radar charts showing key metrics with percentile rankings. They can filter and discover players by position, age, league, team, and statistical thresholds. The tool lets users find similar players by selecting any player and discovering statistically similar alternatives, which is useful for understanding player profiles and finding comparable talents. Users can explore metrics through interactive scatter plots and understand what advanced stats mean through the visualizations. Finally, they can analyze players by position, viewing position-specific metrics that matter most for each role.


EXAMPLE INSIGHTS

The visualization will help answer questions like "Which young midfielders under 23 have the best expected goal contributions per 90 minutes?" to identify breakout talents before they become household names. Users can ask "How does Salah compare to other Premier League wingers this season?" to contextualize star players against their peers. They can explore "Which defenders provide the most progressive passing?" to find ball-playing center-backs suited for possession-based tactics. And they can discover "Who are the most clinical finishers outperforming their expected goals?" to find players who convert chances at elite rates.


DATA SOURCE

The primary dataset is Football Players Stats 2024-2025 from Kaggle, originally sourced from FBref (Football Reference).

Dataset link: https://www.kaggle.com/datasets/hubertsidorowicz/football-players-stats-2024-2025

It contains over 250 columns of statistics for approximately 2,500 players and is updated weekly in CSV format.
...
Data processing will use Python with Pandas. The primary visualization tool will be Altair for interactive web-based charts. The application will be hosted on the Berkeley iSchool server.
